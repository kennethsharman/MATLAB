...
t_domain = find(t>0 & t<6.0); % time domain for plot
plot(t(t_domain),audio_in(t_domain)); 
xlabel('Time(s)'); ylabel('Amplitude')
title('Acoustic A3 Waveform')

subplot(1,2,2) % second plot
t_domain = find(t>0.145 & t<.185); % time domain for plot
plot(t(t_domain),audio_in(t_domain));
xlabel('Time(s)'); ylabel('Amplitude')
title('Magnified View of Acoustic A3 Waveform')
...
axis([0 2400 0 1.2*10^-2]); xticks(linspace(200,2400,12));
